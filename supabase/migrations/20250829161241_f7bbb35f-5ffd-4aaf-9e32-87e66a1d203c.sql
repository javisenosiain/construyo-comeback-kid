-- Fix business_settings security vulnerabilities
...
-- Step 3: Add a constraint to ensure user_id always references a valid user
-- This prevents orphaned business settings
ALTER TABLE business_settings 
ADD CONSTRAINT business_settings_user_id_fkey 
FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;

-- Step 4: Create additional audit trigger for business_settings access
...
-- Step 5: Create trigger to audit business settings access
...
-- Step 6: Update RLS policy to be more explicit and secure
...
-- Step 7: Add index for better performance on user_id lookups
